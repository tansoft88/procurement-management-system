<fieldset>
	...
	<tr>
		...
		<?php if($system_group_id == 1){ ?>
		<td class="actions">
		<?php if(($tenderResponse['TenderResponse']['approved'])==0){ ?>
			<?php echo $this->Form->postLink(__('Approve'), array('action' => 'approve_quote', $tenderResponse['TenderResponse']['id']), null, __('Are you sure you want to approve Response # %s?', $tenderResponse['TenderResponse']['id'])); ?>
		<?php } ?>
		</td>
		<?php } ?>
	</tr>
...
<?php
	echo $this->Form->create('TenderResponse',array('action'=>'index_pdf'));
